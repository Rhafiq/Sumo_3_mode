...
void loop() {
  ...
  back(); //geri means back. For turn back we need to create this function.
  if (page == 1) {

    myOLED.clrScr();
    myOLED.invertText(true);
    myOLED.print("MAIN MENU", CENTER, 0); // MANI MENU
    myOLED.invertText(false);
    myOLED.print("INVERT MODE:", 10, 10); //INVERT MODE
    myOLED.print("BRIGHTNESS", 10, 25); //BRIGHTNESS
    myOLED.print("ROBOT MODE", 10, 40); //MENU3
    if (inv2 == true && page == 1)
  ...
  select; //SELECTION BUTTON FUNCTION
  ...
  {
    myOLED.clrScr();
    myOLED.print("BRIGHTNESS:", 0, i); //BRIGHTNESS : 
    myOLED.printNumI(m, 60, 25);
    myOLED.update();
    if (y == 1 && page == 3)
      brightness;  //brightnessSetting function
  }
...
void brightness //brightnessSetting function
{
  int y = digitalRead(3);
  if (y == 0)
    diger3 = 0;
...
void down()  // down function. When button pressed selection sign ">" will move downwards.
{ int x = digitalRead(2);
  if (x == 0)
    diger = 0;
  if (x == 1 && diger == 0)
